...
public class AssetsManager : Manager
{
    ...
    public AssetsManager CreateSharedContext(string contextName)
    {
        if (!_namedContexts.TryGetValue(contextName, out var context))
        {
            ...
            Logger.LogInfo($"Created shared asset context: {contextName}");
        }
        return context;
    }
    ...
    internal AssetBundle? LoadBundle(PluginAssets controller, string bundlePath)
    {
        if (!_loadedBundles.TryGetValue(bundlePath, out var bundleRef))
        {
            ...
            Logger.LogInfo($"Loaded AssetBundle: {bundlePath}");
        }
    ...
    internal void ReleaseBundle(PluginAssets controller, string bundlePath, bool unloadAllLoadedObjects)
    {
        ...
        if (bundleRef.ReferenceCount == 0)
        {
            ...
            Logger.LogInfo($"Unloaded AssetBundle: {bundlePath} (ref count reached 0)");
        }
        else
        {
            Logger.LogInfo($"Released bundle reference: {bundlePath} (ref count: {bundleRef.ReferenceCount})");
        }
    }
    ...
    internal void ReloadBundle(PluginAssets controller, string bundlePath)
    {
        if (!_loadedBundles.TryGetValue(bundlePath, out var bundleRef))
        {
            Logger.LogWarning($"Cannot reload bundle that isn't loaded: {bundlePath}");
            return;
        }

        Logger.LogInfo($"Reloading bundle: {bundlePath}");
        ...
        if (newBundle == null)
        {
            Logger.LogError($"Failed to reload AssetBundle: {bundlePath}");
            _loadedBundles.Remove(bundlePath);
            return;
        }
        ...
        Logger.LogInfo($"Successfully reloaded bundle: {bundlePath}");
    }
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        if (disposing)
        {
            Logger?.LogInfo($"Unloading {_loadedBundles.Count} bundles...");
