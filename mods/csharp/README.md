...
- `Directory.Build.props` holds shared MSBuild defaults for all projects under `mods/csharp/`.
...
2. Run the modding environment setup script:
   ...
   - Copy required game assemblies to `mods/csharp/.local/game-refs/managed/`
...
3. (Optional) Review the generated `.env` and `mods/csharp/GameDir.props` files

## Build

- `rush build --to @tvs/mods-csharp`
...
If `TVSAutoDeployOnBuild` is true in `mods/csharp/GameDir.props`, plugin projects are copied
to `BepInEx/plugins` automatically after each successful build.

## Line endings

This repository uses LF as the source-of-truth line ending for tracked text files, including C# and MSBuild files.

- Keep `core.autocrlf` set to `false` in this repo.
- Let `.gitattributes` and `.editorconfig` enforce LF for `.cs`, `.sln`, `.csproj`, `.props`, and Unity text assets.
- `*.cmd` and `*.bat` are the intentional CRLF exception for Windows command shell compatibility.
...
The script source is `mods/csharp/scripts/mod-manager.ps1`.

## Assembly management

### Local cache and game references

Game assemblies are copied to `mods/csharp/.local/game-refs/managed/` during setup.
...
The first implementation lives at `mods/csharp/TVS.SampleMod/assets/assets.manifest.json` and supports any mix of:

- asset bundles
- loose runtime files
- extra assembly dependencies

Manifest files are validated by `mods/csharp/scripts/validate-mod-assets.ps1`.

Validation checks include:

1. JSON schema conformance against `mods/csharp/schemas/assets.manifest.schema.json`
...
GitHub Actions can also populate `mods/csharp/.local/game-refs/managed/` from the private `fealyx/tvs-build-data` repository.
The CI helper is `mods/csharp/scripts/fetch-tvs-build-data.ps1`.
...
4. Extract the bundle to the same local cache path used by `setup-modding-env.ps1`, then generate `mods/csharp/GameDir.props` pointing `TVSManagedDir` at that cache.
...
1. Create `mods/csharp/TVS.YourMod/TVS.YourMod.csproj`.
